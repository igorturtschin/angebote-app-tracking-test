...
E-commerce — воронка офферов и кнопки на экране оффера — раздел 4, ещё не
заполнен.
...
Статус: **сделано.** Собрано и проверено на эмуляторе 2026-09-06: события
дошли в Amplitude с обоими свойствами и в ожидаемом порядке.
...
из Activity, а все 5 экранов приложения живут в одной Activity, поэтому
...
| `current_offer` | Приложение 1 → *Значения трекинга*, колонка `current_offer` |
...
**Решение.** В этом приложении цена не оправдана. Навигация — список ⇄ оффер
и больше ничего, поэтому предыдущий экран почти целиком выводится из самого
`screen_name`: у экрана оффера предыдущий всегда `Startseite`, у стартового —
либо старт приложения, либо один из четырёх офферов. Свойство повторяло бы
структуру приложения, а не сообщало новое.

Пересмотреть, когда появится третий тип экрана или несколько путей к одному
экрану. Задним числом свойство не добавляется: на уже собранных событиях его
не будет.
...
- при переходе на экран оффера;
- при **каждом возврате** на экран: назад с оффера на стартовый и назад в
  приложение из браузера или из другого приложения.
...
| Экран (`screen_name`) | Тип | `current_offer` | ID оффера | Позиция |
|---|---|---|---|---|
| `Startseite` | стартовый | `Neustarter & Highlights` | — | — |
| `Fitwerk` | оффер | `20 % auf alle Sportschuhe` | `offer_01` | 0 |
| `Nordlicht Wohnen` | оффер | `15 € Rabatt ab 75 € Bestellwert` | `offer_02` | 1 |
| `Kaffeekontor` | оффер | `Versandkostenfrei bestellen` | `offer_03` | 2 |
| `Sichtbar Optik` | оффер | `2 für 1 auf Brillengläser` | `offer_04` | 3 |

- На экране оффера `screen_name` совпадает с названием магазина.
- `current_offer` на экране оффера — заголовок этого оффера; на стартовом
  экране — название списка, `Neustarter & Highlights`. Оно есть только в
  трекинге: на самом экране заголовок другой, **Angebote**.
- Позиция — место карточки на стартовом экране, первая `0`. Именем свойства
  она станет в разделе 4.
