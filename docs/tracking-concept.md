...
(ответвлена от `v1/amplitude`, тот же проект Amplitude, тот же SDK).
SDK аналитики: **Amplitude, Android-Kotlin SDK**.

**Эта версия описывает установку SDK, открытие приложения и экраны.**
E-commerce — воронка офферов и кнопки на экране оффера — раздел 4, на
Amplitude ещё не начат.

**Что нового в приложении v2** (Приложение 1): нижняя навигация, восемь
офферов вместо четырёх, разбитых на четыре категории, два списка офферов на
стартовом экране (Highlights и Neustarter, два оффера стоят в обоих) и пять
одинаковых тест-страниц для отработки path- и funnel-чартов.
...
**2.0 — 2026-09-06.** Приложение v2, ветка `v2/amplitude` (ответвлена от
`v1/amplitude`). Приложение 1 переписано под новый состав: нижняя
навигация, восемь офферов в четырёх категориях, два списка на стартовом
экране (Highlights — четыре оффера, Neustarter — шесть; Fitwerk и
Kaffeekontor стоят в обоих), пять тест-страниц `Testseite 1`–`Testseite 5`.
Раздел 3: в *Values* и *Trigger* добавлены тест-страницы (несут только
`screen_name`) и переходы по нижней навигации; подраздел *Почему нет
`previous_screen`* переписан — путей в оффер теперь несколько, но свойство
всё равно не шлём. Раздел 4 на Amplitude по-прежнему не начат. Из
Приложения 2 убран подраздел про параметр `plan`. Счёт экранов в разделах
1 и 3 и в Приложении 2 приведён к 14.

**1.2 — 2026-09-06.** Починка поворота экрана. Приложение 1, *Известные
проблемы*: баг закрыт — открытый оффер и состояние его экрана переживают
пересборку. Раздел 3, *Trigger*: переписан абзац про поворот — `screen_view`
на поворот больше не уходит, и это правило трекинга, а не побочный эффект
поведения приложения. Приложение 2, *Автозахват `Application Opened`*:
помечено, что нашего `screen_view` поворот больше не касается.

**1.1 — 2026-09-06.** Добавлены раздел 2 (открытие приложения — ручного
события нет), раздел 3 (`screen_view`) и пустой раздел 4 (e-commerce).
В Приложении 1 таблица офферов стала таблицей *Значения трекинга* и
получила строку стартового экрана. В Приложение 2 добавлен вопрос о
`screen_view` при возврате в приложение. В разделе 1 исправлено
противоречие: `newDeviceIdPerInstall` стоял в списке параметров, которые
«выключают то, что и так выключено», хотя его значение `true` — наоборот,
единственное отступление от умолчания. По итогам отладки на эмуляторе
2026-09-06 уточнены свойства и триггер в разделе 2, а в Приложение 2 добавлен
раздел про поворот экрана и порядок событий автозахвата.
...
# 2. Открытие приложения — ручного события нет

Статус: **сделано** — кодом делать нечего.

## Why

Открытие приложения отмечает начало визита: оно выходит каждый раз, когда
приложение попадает на передний план. По нему считают, как часто возвращаются
и что делают сразу после возврата.

Amplitude собирает это сам. Автозахват `APP_LIFECYCLES` (включён, раздел 1)
даёт `[Amplitude] Application Opened`, а вместе с ним `Application
Installed`, `Updated` и `Backgrounded`. Первый запуск 2026-09-04 это
подтвердил.

Поэтому ручного эквивалента Firebase-события `app_open` на этой ветке
**нет**: он дублировал бы автозахваченное событие. Кода это не стоит совсем —
на ветке Firebase то же самое потребовало наблюдателя за жизненным циклом
процесса и своего вызова. Но обмен не бесплатный: момент выбирает SDK, и
выбирает он шире, чем выбрали бы мы, — см. *Trigger*.

## Code

Кода нет. Событие включает строка `AutocaptureOption.APP_LIFECYCLES` в
конфигурации SDK, раздел 1, шаг 5.

## Values

Своих значений нет: свойства события задаёт SDK. Он кладёт на него три
(проверено на данных 2026-09-06):

| Свойство | Значение |
|---|---|
| `[Amplitude] From Background` | `false` при холодном старте, `true` при возврате из фона |
| `[Amplitude] Version` | `versionName` приложения |
| `[Amplitude] Build` | `versionCode` приложения |

`From Background` частично закрывает `trigger_source` из списка кандидатов
Firebase: холодный старт от возврата оно отличает, а вот **как** человек
вернулся — нет.

Остальные два кандидата (`background_duration_sec`, `previous_screen`) на
это событие **своими полями не навесить**. `[Amplitude] Application Opened`
строит SDK, и дописать в него свойство можно только enrichment-плагином,
который перехватывает событие перед отправкой. Такой плагин уже нужен для
другой задачи — см. [Приложение 2](#приложение-2--открытые-вопросы),
*Frustration-события*. Пока его нет, своих параметров нет.
...
- **выходит и на поворот экрана**: поворот даёт пару `Application
  Backgrounded` + `Application Opened`. Это отличие от ветки Firebase, где
  `app_open` слался руками с наблюдателя за жизненным циклом процесса и на
  поворот не срабатывал. Автозахват дешевле, но шумнее — подробности в
  [Приложении 2](#приложение-2--открытые-вопросы), *Автозахват `Application
  Opened`: поворот и порядок событий*;
...
Статус: **в работе.** Стартовый экран и экраны офферов — сделано и
проверено на эмуляторе 2026-09-06 (события дошли в Amplitude с обоими
свойствами и в ожидаемом порядке). Тест-страницы `Testseite 1`–`Testseite 5`
и переходы по нижней навигации — код приложения v2 ещё не написан; правило
для них ниже.
...
Берём **своё имя**. Три довода:

1. Имена в квадратных скобках принадлежат SDK. Заполнять их своими руками —
   значит писать в чужой словарь: если SDK когда-нибудь начнёт заполнять эти
   поля сам, его значения и наши окажутся в одном определении, и разделить
   их будет нечем.
2. Довод за схему автозахвата — «ручные и автоматические события сольются в
   одно определение, если `SCREEN_VIEWS` включат». В этом приложении их не
   включат: автозахват здесь принципиально нерабочий (одна Activity), а не
   отложенный.
3. Свойство `screen_name` уже выбрано в [Приложении 2](#приложение-2--открытые-вопросы)
   для enrichment-плагина, который проставит имя экрана на все события. Одно
   имя свойства на весь проект, а не два синонима об одном и том же.

Цена решения: событие не совпадает со схемой Mobile Autocapture и стоит в
интерфейсе Amplitude рядом с событиями SDK как обычное кастомное. Это и есть
намерение.
...
Значения в примере — экран оффера `offer_01`. Каждое событие несёт значения
своего экрана, см. *Values*.
...
На тест-страницах `screen_view` несёт **только `screen_name`**, без
`current_offer`. На этих экранах нет оффера, и значение было бы либо пустым,
либо неверным. Это та же логика, что с `SCREEN_CLASS` выше: свойство без
...
## Почему нет `previous_screen`

Свойства «предыдущий экран» **не шлём**.

**Что отвечает на вопрос «откуда пришли» и без него.** Каждый `screen_view` —
отдельное событие со своим `screen_name`, и на всех событиях стоит
`session_id` (автозахват `SESSIONS`). Этого достаточно чартам, которые
строят путь:

- **Journeys / Pathfinder** сшивают события по пользователю внутри сессии,
  порядок — по `client_event_time`. Путь, который «заканчивается на
  `screen_view`», показывает, что было перед ним. Это прямой ответ на вопрос
  о предыдущем экране. Оговорка: шаги пути — события, а не экраны, поэтому
  между двумя `screen_view` в пути окажутся и другие события сессии; можно ли
  ограничить путь одним типом событий, в справочнике не проверено.
- **Behavioral cohort** через `...then` собирает «был на A, потом на B» — но
  `then` не требует, чтобы B шёл сразу за A.

**Чего без свойства нет.** `previous_screen` как простой размерности —
колонки, по которой можно разбить или отфильтровать любой чарт, не строя
path-чарт. Например «сколько просмотров стартового экрана пришло с экрана
оффера» одним group by в Event Segmentation. В path-чартах это есть, но
path-чарт отвечает медленнее и хуже кладётся в дашборд.

**Решение.** Не шлём. В v2 в оффер ведут два пути — карточка в списке на
стартовом экране и кнопка на тест-странице, — то есть условие «несколько
путей к одному экрану», при котором в v1 предлагалось вернуться к вопросу,
наступило. Решение от этого не меняется:

- на вопрос «откуда пришли» отвечает path-чарт (Journeys / Pathfinder):
  путь, заканчивающийся на `screen_view` оффера, показывает предыдущий
  экран, и тест-страница видна в нём отдельной веткой;
- `previous_screen` как простая размерность стоил бы приложению состояния
  «предыдущий экран», которое сейчас наружу не торчит, а выигрыш — только
  скорость того же разреза в дашборде;
- различие «пришёл из списка / пришёл с тест-страницы» в разделе 4 будет
  видно и без него: у оффера, открытого с тест-страницы, не будет
  контекста листа.

Задним числом свойство не добавляется: на уже собранных событиях его не
будет. Пересмотреть, если появится путь, который path-чарт не разделяет.
...
Короткий возврат считается тоже. Одно событие на один визит на экран:
вернулся — новое событие.

Нижняя навигация сама по себе события не порождает: клик по её кнопке
проверяется только тем, что на целевом экране вышел `screen_view`.
Тест-страницы для трекинга ничем не отличаются от остальных экранов, кроме
`screen_name` и отсутствия `current_offer`.

**Не слать:** на построение экрана, которого пользователь ещё не видит; на
прокрутку; на перерисовку того же экрана без смены экрана, при повороте экрана.

Возврат из браузера, из скачивания и переключение приложений — формально не
просмотр, а возврат. Событие всё равно шлём; как отделить эти случаи в
отчётах — [Приложение 2](#приложение-2--открытые-вопросы),
*`screen_view` при возврате в приложение*.

Поворот экрана `screen_view` **не даёт**: телефон при повороте пересоздаёт
экран, но пользователь с него не уходил и на него не возвращался — он всё
это время смотрел один и тот же экран, и в аналитике должен смотреть один.
Это отличается от возврата в приложение (абзац выше): туда пользователь
вернулся сам, поэтому `screen_view` нужен — в том числе когда за время в
фоне телефон выгрузил приложение из памяти и экран пришлось строить
заново.
...
Раздел переводится с ветки Firebase отдельной задачей: цепочка
`view_item_list` → `select_item` → `view_item` → `begin_checkout` плюс
четыре кастомных события (`generate_code`, `copy_code`, `go_to_shop`,
`download_coupon`).

Источники для перевода:

- `infos/tracking-concept v1 firebase ga4.md`, раздел *4. E-commerce events*
  и Приложение 2 — что мы шлём на ветке Firebase и что там осталось
  нерешённым;
- `infos/amplitude-события-и-чарты.md`, §5 — сквозной кейс «товар в листе →
  клик → begin_checkout» на Amplitude, два способа собрать отчёт (воронка с
  hold constant и persisted property) и §5.5 — что посчитать нельзя;
- там же §1.4 и §5.2 — массив объектов как способ передать список товаров.

Что уже известно и определяет перевод: зарезервированных ecommerce-имён
событий у Amplitude нет, стандартного свойства для списка и для позиции в
списке тоже — всё кастомное; массив объектов работает только при включённом
property splitting.
...
Тест-страницы нужны для path- и funnel-чартов. Переходами между ними строят
цепочку экранов; кнопкой в оффер — путь в оффер, на котором нет контекста
листа. Оффер, открытый с тест-страницы, в разделе 4 не будет привязан ни к
Highlights, ни к Neustarter, и целевое действие в нём попадёт в отчётах по
листам в отдельную группу «без листа». Смысл — получить в одном проекте
...
Четыре категории, по два оффера в каждой. Категория в события пока не
уходит; параметром (`item_category`) станет в разделе 4.

### Списки офферов

Два списка, оба на стартовом экране. `item_list_id` / `item_list_name` —
значения для раздела 4, сейчас в события не уходят.
...
- Fitwerk (`offer_01`) и Kaffeekontor (`offer_03`) стоят в обоих списках,
  и позиция у них в каждом своя. Именем свойства позиция станет в разделе 4.
...
**v2 не собран.** Нижняя навигация, второй блок офферов и тест-страницы —
описаны здесь, кода ещё нет. Правила поворота и возврата из раздела 3
распространяются на новые экраны так же, как на прежние, но на устройстве
это не проверялось. Раздел 3, *Status* — `в работе`.

**Решено 2026-09-06 — поворот экрана.** Раньше поворот сбрасывал приложение
на стартовый экран: открытый оффер и состояние его экрана (показан ли код
купона, какие кнопки нажаты) не переживали пересоздание экрана. Теперь
переживают. Заодно поворот перестал слать лишний `screen_view` — раздел 3,
*Trigger*. Воспроизводилось на эмуляторе 2026-09-01, исправление проверено
на эмуляторе 2026-09-06. Как это подключено в коде — `implementation-notes.md`,
Part 1.
...
**v1 закончена.** Собрана, установлена и проверена руками на настоящем
Pixel 10a (Android 17) и на нескольких эмуляторах, включая планшет.
Проверено: стартовый экран, экран оффера, генерация кода, смена цвета
использованных кнопок, скачивание PDF в «Загрузки» и click-out в браузер.

**v2 не начата.** Нижняя навигация, восемь офферов в четырёх категориях,
два блока на стартовом экране и пять тест-страниц — описаны в этом
документе, кода на ветке `v2/amplitude` ещё нет.
...
1. **Чем помечать.** Возвратному `screen_view` нужен признак, по которому
   ...
   он завышает числа. Кандидаты: булево свойство на самом событии
   (приложению придётся отличать возврат от первого показа) или
   `previous_screen` (раздел 3, *Почему нет `previous_screen`*) — у возврата
   предыдущий экран совпадает с текущим. Второй вариант дешевле по коду, но
   тянет за собой свойство, от которого раздел 3 отказался.
