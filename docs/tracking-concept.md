...
Поворот экрана отдельного правила не требует: приложение при повороте
сбрасывается на стартовый экран (Приложение 1, *Известные проблемы*),
поэтому после поворота честно приходит `screen_view` со `Startseite`. Это
поведение приложения, а не трекинга.
