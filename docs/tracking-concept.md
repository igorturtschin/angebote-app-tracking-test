...
Статус: **в работе.** Шаги 1–7 собраны и проверены сборкой 2026-09-04.
Шаг 8 — плагин `screen_name` — ещё не написан.
...
проверено на эмуляторе 2026-09-06 (события дошли в Amplitude со
`screen_name` и `current_offer` и в ожидаемом порядке). Тест-страницы
`Testseite 1`–`Testseite 5` и переходы по нижней навигации — код приложения
v2 ещё не написан; правило для них ниже. `previous_screen_name` — новое
свойство, в коде его пока нет.
...
Статус: **в работе.** Цепочка `view_item_list` → `begin_checkout` встроена
и проверена на эмуляторе 2026-09-07 (`implementation-notes.md`,
*E-commerce events*). Четыре события кнопок и подраздел *DA — настройка в
Amplitude* — не сделано.
...
У оффера, открытого с тест-страницы, листа нет — тогда событие несёт только
массив, а в объекте нет `index`: позиция описывает место в листе, которого
не было.
...
**Осталось:** плагин `screen_name` и `previous_screen_name` (разделы 1 и 3),
четыре события кнопок и настройка проекта в Amplitude (раздел 4, *DA —
